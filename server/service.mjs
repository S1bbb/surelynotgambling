...
export function initialState() {
  return { balance: 1000000, rtpBps: 9700, version: 1, seed: newSeed(), nonce: 0, rounds: [], retired: [], audit: [] };
}
export function snapshot(s) {
  return { balance: s.balance, rtpBps: s.rtpBps, version: s.version, commitment: hash(s.seed), nonce: s.nonce, rounds: s.rounds.slice(-100).reverse(), retired: s.retired, audit: s.audit.slice(-50).reverse(), stats: { count: s.rounds.length, wagered: s.rounds.reduce((a,r)=>a+r.amount,0), paid: s.rounds.reduce((a,r)=>a+r.payout,0) } };
}
...
export function placeBet(s, b) {
  ...
  const round = { id: randomUUID(), requestId: b.requestId, amount: b.amount, threshold: b.threshold, direction: b.direction, clientSeed: b.clientSeed, version: s.version, commitment: hash(s.seed), nonce, result, rtpBps: s.rtpBps, ...terms(b.direction,b.threshold,s.rtpBps), ...outcome, balance: s.balance, createdAt: new Date().toISOString() };
  s.rounds.push(round);
  return round;
}
